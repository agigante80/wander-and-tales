...
2. The contributor always produces **text content and image prompts**, and **may
   optionally also propose pictures**, either generated with their own key or their
   own original art. Built PDFs stay out of the PR (CI and the maintainer build
   those derived artifacts).
3. The maintainer's **OpenAI key is a repository secret**, used only on the trusted
   ...
   A contributor who proposes AI art uses their own key, never the maintainer's.
4. For a story whose author did not propose art, the illustrations are produced
   **after acceptance** by the maintainer's workflow. Art a contributor did propose
   is reviewed like any other change and kept, replaced, or regenerated by the
   maintainer.
...
- Requiring art, or committing built PDFs, inside a contributor PR. Proposing
  pictures is optional and welcome; PDFs are derived artifacts that CI and the
  maintainer build.
...
Contributor (Claude Code + a GitHub account)
   |
   v
[1] create-story skill  ->  draft PR (text + prompts; art optional, no built PDFs)
   |
   v
...
5. **Write the content.** Draft en-GB first (the source of truth), then every other
   ...
   and `canon/`. Image entries always include **prompts**.
6. **Pictures, optionally.** The skill offers the author a choice: submit prompts
   only (the maintainer illustrates after acceptance, needing no contributor key);
   generate the art now with the author's **own** OpenAI key (`generate-images`); or
   drop in their own original pictures. Any art the author supplies is written to the
   story or world `assets/`, and the PR template asks them to confirm it is theirs to
   give. The maintainer's key is never touched here.
7. **Advisory consistency check.** The skill reviews the new story's use of canon
...
8. **Validate and preview.** Run `python -m build validate` and `python -m build
   lint`, fixing structural issues until clean (image-file warnings are expected and
   fine). Build a kit PDF with `python -m build render` so the author sees their
   work, and regenerate `catalog.md`.
9. **Submit, or stop.** Two modes:
   - **Contribute:** create a branch, commit the text content and any pictures the
     author chose to propose (but not built PDFs), and open a **draft PR** with a
     filled-in template (the quality checklist, the confirmation that supplied art is
     theirs to give, and a note that non-English text is machine-drafted and needs
     native review). The skill uses the `gh` CLI or the GitHub MCP; if neither is
     authenticated it prepares the branch and prints the exact steps.
   - **Keep it for yourself:** stop after step 8 with a local kit. No PR.
...
warnings are allowed). It then builds the changed story's kit PDFs (illustrated if
the author proposed art, otherwise text only, since the renderer skips missing
images) and uploads them as a workflow artifact, so the maintainer can download and
eyeball a real kit without building anything locally. Determining the "changed story" can be
...
commits the new art and PDFs to `main` with a clear message. Because
`generate-images` skips images whose files already exist (unless forced), a story
where the contributor already proposed and the maintainer accepted art is left as
is; this workflow fills in only the missing illustrations. The maintainer can
...
- **PR template** at `.github/pull_request_template.md`: the authoring quality
  ...
  that any non-English text is machine-drafted and needs native review, and, when
  the author proposes pictures, a checkbox confirming the art is theirs to give
  (their own original work, or art they generated, or otherwise licensed for an open
  project).
...
- **No OpenAI key required.** Text and a preview PDF do not need it, and a
  prompts-only submission is illustrated later by the maintainer. A key is needed
  only if the author chooses to generate AI pictures themselves; supplying their own
  original art needs no key either.
...
- The PR carries **no built PDFs**, but **may include proposed art** (the author's
  choice, made with their own key or their own pictures). Text, prompts, and the
  regenerated catalog.md are always included.
